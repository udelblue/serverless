...
1. From the command line, use AWS SAM to build and deploy the AWS resources for the pattern as specified in the template.yml file:
    ...
    ```
    sam build
    sam deploy --parameter-overrides SourceBucketName=testdohsrcbucket DestinationBucketName=testdohdesbucket --guided
    ```
...
1. Navigate to \layers\python and run the command below:
    ```bash
    (current directory) $ pip install -r requirements.txt --target lib/python3.8/site-packages
    ```
